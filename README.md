...
Два плагина студии **ZS Records** в одном комплекте. Общая кодовая база, намеренно
противоположный замысел.

| | ZS-motion | ZS-MOTION-FAN |
|---|---|---|
| Органов управления | 24 | **10** |
| Вентилятор | лопастная **амплитудная** модуляция | настоящая **кольцевая** — тон вырезается |
| Хорус | два голоса 11 и 15 мс + обратная связь | один голос **5.6 мс**, без петли |
| Форма LFO | список из шести | непрерывный **Morph** |
| Rate | 0.01–10 Гц логарифмически | **0–32 Гц** линейно |
| Оверсэмплинг | 2×/4× | нет, и латентности нет |
| Интерфейс | кинетическая скульптура | плоская развёртка несущей |

Младший построен **по измерениям** сторонней референсной реализации, снятым
собственным стендом (`Tests/Compare.cpp`) — по входу и выходу, без вскрытия
бинарников. Что из этого получилось и где остались расхождения — в разделе
[Что показал стенд](#что-показал-стенд).
...
ZS-MOTION-FAN строился не по документации референса, а по его выходу — и прибор
трижды поправил догадки, которые казались очевидными.

**Вентилятор — кольцевая модуляция, а не амплитудная.** Тон 1 кГц на выходе
пропадает (−99 dB), а боковые стоят гребёнкой с шагом ровно в Rate: при 2 Гц
звенят все чётные смещения, все нечётные молчат. Значит несущая — сам LFO. Нули
каждые 5 гармоник дают скважность 1/5.

**Форму несущей пришлось переделать.** Первая версия — прямоугольник со
сглаживанием. Нули совпали, огибающая нет: у прямоугольника гармоники падают
как 1/k, а измеренные **плоские до 19-й и потом обрыв**. Плоский спектр с нулями
каждые 5 — подпись **пары противоположных импульсов** (две кромки лопасти, а не
её плоскость). Переписано на аддитивную банд-лимитированную таблицу; зануление
несущей выросло 49 → 66 dB.

**Мануал занижает Rate втрое.** Заявлено 0–10 Гц, измерено **0–32 Гц** линейно.

**`Active` — мастер-выключатель.** При `Active = 0` референс отдаёт чистый сухой
сигнал, вентилятор тоже молчит. «Использовать вентилятор отдельно» у него значит
погасить кнопку режима, а она не автоматизируется. Отсюда режим **`None`** —
у нас он автоматизируется.

**Что не сошлось.** У референса есть слабые провалы на 8-й и 12-й гармониках,
которых нет у нас, — одной парой импульсов они не объясняются, природа не
разгадана. И зануление несущей 66 dB против >100 dB. На слух это ниже порога, но
«идентично» сказать нельзя.

**Границы.** Стенд работает только по входу и выходу: никакого разбора
бинарников, ресурсов или интерфейса. Оформление референса рисовало сторонее
агентство — оно не воспроизводилось; здесь своя графика ZS.
